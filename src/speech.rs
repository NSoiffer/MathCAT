...
use sxd_xpath::{Context as XPathContext, Factory, Value, XPath};
...
impl MyXPath {
    ...
    fn is_true(&self, context: &XPathContext, mathml: Element) -> Result<bool> {
    ...
    pub fn evaluate<'c>(&self, context: &XPathContext<'c>, mathml: Element<'c>) -> Result<Value<'c>> {
        // debug!("evaluate: {}", self);
...
impl SpeechPattern  {
    ...
    fn is_match(&self, context: &XPathContext, mathml: Element) -> Result<bool> {
...
struct ContextStack<'c> {
    ...
    base: XPathContext<'c>                      // initial context -- contains all the function defs and pref variables
}
...
impl<'c, 'r> ContextStack<'c> {
    ...
    fn base_context(var_defs: PreferenceHashMap) -> XPathContext<'c> {
...
impl<'c, 's:'c, 'r, 'm:'c> SpeechRulesWithContext<'c, 's,'m> {
    ...
    pub fn get_context(&mut self) -> &mut XPathContext<'c> {
