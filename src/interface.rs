...
pub fn set_mathml(mathml_str: String) -> Result<String> {
    ...
    return MATHML_INSTANCE.with(|old_package| {
        let (new_package, canonicalized_mathml) = create_mathml_instance_with_text(&CanonicalizeContext::new_from_global_prefs_cached(), &mathml_str)?;
...
pub fn get_intent<'a>(mathml: Element<'a>, doc: Document<'a>) -> Result<Element<'a>> {
    ...
    let mathml = cleanup_mathml(&CanonicalizeContext::new_from_global_prefs_cached(), mathml)?;
