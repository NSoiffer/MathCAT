...
use crate::canonicalize::{as_element, name, CanonicalizeContext};
...
use crate::stateless_interface::{cleanup_mathml, create_mathml_instance_with_text};
...
pub fn set_mathml(mathml_str: String) -> Result<String> {
    ...
    crate::speech::SPEECH_RULES.with_borrow_mut(|rules| rules.read_files())?;

    return MATHML_INSTANCE.with(|old_package| {
        let (new_package, canonicalized_mathml) =
            crate::definitions::SPEECH_DEFINITIONS.with_borrow(|definitions|
                create_mathml_instance_with_text(&CanonicalizeContext::new_from_global_prefs_cached(), definitions, &mathml_str)
            )?;
        old_package.replace(new_package);
        return Ok(canonicalized_mathml);
    });
}
...
pub fn get_intent<'a>(mathml: Element<'a>, doc: Document<'a>) -> Result<Element<'a>> {
    ...
    let mathml = crate::definitions::SPEECH_DEFINITIONS.with_borrow(|definitions|
        cleanup_mathml(&CanonicalizeContext::new_from_global_prefs_cached(), definitions, mathml)
    )?;
...
mod tests {
    ...
    use super::*;
    use crate::element_util::is_same_doc;
